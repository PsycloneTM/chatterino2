                                  ...
                                  MessageElementFlags flags)
{
    if (flags.hasAny(this->getFlags()))
    {
        if (flags.has(MessageElementFlag::EmoteImages))
        {
            ...
            QSize size;
            if (this->getFlags().has(MessageElementFlag::BttvModifierWide))
            {
                size = QSize(int(container.getScale() *
                                 BTTV_WIDE_EMOTE_SIZE.width() * emoteScale),
                             int(container.getScale() *
                                 BTTV_WIDE_EMOTE_SIZE.height() * emoteScale));
            }
            else
            {
                size = QSize(
    ...
    LayeredEmoteElement::getLoadedImages(float scale)
{
    ...
    for (const auto &emote : this->emotes_)
    {
        ...
        if (emote.flags.has(MessageElementFlag::BttvModifierWide))
        {
            size = BTTV_WIDE_EMOTE_SIZE;
        }
